...
SPECIAL_MAPPING = {
    "NEIROETHUSDT": "NEIROUSDT", "1000RATSUSDT": "RATSUSDT",
    "1000RATS": "RATS", "1000WHYUSDT": "WHYUSDT", "1000WHY": "WHY",
    "1000XUSDT": "XUSDT", "1000X": "X", "1000000MOGUSDT": "MOGUSDT",
    "1000000MOG": "MOG", "SOONUSDT": "SOON"
}
...
def process_binance_indices():
    ...
    for sym in symbols:
        ...
        if "binance" in df["exchange"].values:
            try:
                w = float(df.loc[df["exchange"] == "binance", "weight"].iloc[0])
                if w > 0.5:
                    continue
            except Exception:
                continue
...
if __name__ == "__main__":
    ...
    # output_path = f"target_csv/{datetime.now().strftime('%Y%m%d')}.csv"
    result_df.to_csv("output.csv", index=False)
    # logger.info(f"最终生成 {len(result_df)} 个符号 -> {output_path}")
